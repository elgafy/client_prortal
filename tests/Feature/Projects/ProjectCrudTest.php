...
test('client-role users cannot access project management', function () {
    ...
    $this->actingAs($clientUser)->get(route('projects.index'))->assertForbidden();
    $this->actingAs($clientUser)->get(route('projects.create'))->assertForbidden();
    $this->actingAs($clientUser)->post(route('projects.store'), projectPayload())->assertForbidden();
});
...
test('an administrator can create a project for a client', function () {
    ...
    expect($project->amount)->toBe('3500.0000')
...
test('cancelled projects are excluded from the outstanding total but preserved', function () {
    $client = Client::where('name', 'ABC Company')->firstOrFail();
    Project::create(projectPayload(['client_id' => $client->id, 'amount' => '3500.0000']));
    Project::create(projectPayload(['client_id' => $client->id, 'amount' => '1500.0000', 'name' => 'Logo']));
    Project::create(projectPayload(['client_id' => $client->id, 'amount' => '9999.0000', 'name' => 'Cancelled Work', 'status' => Project::STATUS_CANCELLED]));

    $summary = app(ClientAccountService::class)->summary($client);

    expect($summary['currencies']['USD']['projects_total'])->toBe('5000.0000')
        ->and($summary['currencies']['USD']['outstanding'])->toBe('5000.0000')
        ->and($summary['currencies']['USD']['credit'])->toBe('0.0000')
...
test('overpayment produces credit, never a negative outstanding', function () {
    ...
    expect($line['outstanding'])->toBe('0.0000');

    // Exercise the credit branch of the calculation directly.
    $method = new ReflectionMethod($service, 'buildLine');
    $creditLine = $method->invoke($service, '1000.0000', '1250.0000');

    expect($creditLine['net'])->toBe('-250.0000')
        ->and($creditLine['outstanding'])->toBe('0.0000')
        ->and($creditLine['credit'])->toBe('250.0000');
});

test('multi-currency accounts are reported per currency and never combined', function () {
    $client = Client::where('name', 'ABC Company')->firstOrFail();
    Project::create(projectPayload(['client_id' => $client->id, 'amount' => '1000.0000']));
    Project::create(projectPayload(['client_id' => $client->id, 'amount' => '500.0000', 'currency' => 'EGP', 'name' => 'Local Work']));

    $summary = app(ClientAccountService::class)->summary($client);

    expect($summary['has_multiple_currencies'])->toBeTrue()
        ->and($summary['currencies']['USD']['projects_total'])->toBe('1000.0000')
        ->and($summary['currencies']['EGP']['projects_total'])->toBe('500.0000')
...
test('a client with no projects reports zero in its default currency', function () {
    ...
    expect($summary['currencies']['SAR']['projects_total'])->toBe('0.0000')
        ->and($summary['currencies']['SAR']['outstanding'])->toBe('0.0000')
